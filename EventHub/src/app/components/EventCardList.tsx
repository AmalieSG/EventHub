...
export function EventCardList({ event, layout = 'grid', action = 'join' }: EventCardListProps) {
    ...
    if (isGrid) {
        return (
<<<<<<< HEAD
            <article className={articleClasses} aria-label={`Event: ${event.title}`}>
...
<<<<<<< HEAD
                ...
                <ul className="flex flex-col gap-2 text-sm text-gray-400 mt-2 ">
                    <li className="flex flex-wrap gap-x-6 gap-y-2">
                        <span className="flex items-center gap-1"><p className='text-red-400'>🗓️</p>{DateInfo}</span>
                        <span className="flex items-center gap-1"><p className='text-red-400'>⏰</p><p className='font-medium'>{time}</p></span>
                    </li>
                ...
                <menu className="flex gap-3 mt-4 items-center">
                    <button
...
return (
    <article className={articleClasses} aria-label={`Event: ${event.title}`}>
        <figure className="flex w-full h-48">
            <a href={`/events/${event.id}`}  className="relative w-1/3 h-full overflow-hidden rounded-l-xl">
                <img
                    src={event.imageUrl}
            ...
            <section className="flex-1 flex flex-col justify-between p-4">
                ...
                <ul className="flex flex-col gap-y-1 text-xs text-gray-400">
                    <li className="flex flex-wrap gap-x-4">
                        <span className="flex items-center font-medium"><span className="mr-1 text-red-400">⏰</span><p>{time}</p></span>
                        <span className="flex items-center"><span className="mr-1 text-red-400">🗓️</span>{DateInfo}</span>
                    </li>
                ...
                <section className="flex justify-between items-center mt-4 pt-3 border-t border-gray-700/50">
                    ...
                    <menu className="flex items-stretch gap-2">
                        <button
                            onClick={handleActionClick}
            ...
            <section className="flex flex-col gap-2 text-sm text-gray-400 mt-2">
                <dl className="flex flex-wrap gap-x-6 gap-y-2">
                    <div className="flex items-center gap-1">
                        <dt className='text-red-400'>🗓️</dt>
                        <dd>{DateInfo}</dd>
                    </div>
                    <div className="flex items-center gap-1">
                        <dt className='text-red-400'>⏰</dt>
                        <dd className='font-medium'>{time}</dd>
                    </div>
                    <div className="flex items-center gap-1">
                        <dt className='text-red-400'>📍</dt>
                        <address className="not-italic line-clamp-1">{event.address}</address>
                    </div>
                </dl>
            </section>
...
>>>>>>> 3208a5c4e6f84f52e7b55134b4ab881ba8814c1f
   ...
   return (
        <article className={articleClasses} aria-label={`Event: ${event.title}`}>
            ...
            <figure className="relative w-1/3 h-full overflow-hidden rounded-l-xl">
                <img
                    src={event.imageUrl}
                    alt={`Image of ${event.title}`}
                    className="w-full h-full object-cover object-center"
                ...
                <section className="flex flex-col gap-y-1 text-xs text-gray-400">
                    <dl className="flex flex-wrap gap-x-4">
                        <div className="flex items-center font-medium">
                            <dt className="mr-1 text-red-400">⏰</dt>
                            <dd><p>{time}</p></dd>
                        </div>
                        <div className="flex items-center">
                            <dt className="mr-1 text-red-400">🗓️</dt>
                            <dd>{DateInfo}</dd>
                        </div>
                    </dl>
                    <div className="flex items-center mt-1">
                        <span className="mr-1 text-red-400">📍</span>
                        <address className="not-italic line-clamp-1 text-sm">{event.address}</address>
                    </div>
                </section>
