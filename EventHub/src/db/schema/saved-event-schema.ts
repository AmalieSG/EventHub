...
export const savedEvents = sqliteTable(
  ...
  (table) => [
    // Én bruker kan bare lagre samme event én gang
    uniqueIndex("saved_events_event_user_uniq").on(table.eventId, table.userId),
    index("saved_events_user_idx").on(table.userId),
    index("saved_events_event_idx").on(table.eventId),
  ]
);
